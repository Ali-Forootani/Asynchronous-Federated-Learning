...
# -*- coding: utf-8 -*-
"""
Asynchronous Federated Learning with Linear SVM Classifier (Convex Objective)
"""
...
def plot_and_save(array, filename_prefix, title, xlabel, ylabel, log_scale=False):
    ...
    plt.plot(array, label=f"{title} (Client Fraction: {client_fraction*100:.1f}%)")
...
async def main_binary_classification():
    ...
    plot_and_save(max_delays, "svm_max_delays", "Maximum Delay Per Round for SVM", "Round", "Maximum Delay (s)")
    plot_and_save(server_losses, "svm_server_losses", "Server Loss During AFL", "Rounds", "Hinge Loss", log_scale=True)
