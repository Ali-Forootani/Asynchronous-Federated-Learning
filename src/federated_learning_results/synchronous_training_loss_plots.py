...
prefix = "Synchronous_server_losses_clients_10_epochs_50_rounds_400"
...
for file_name in os.listdir(directory):
    if file_name.startswith(prefix) and file_name.endswith(".npy"):
        ...
        print(file_name)
        print("=======================")
        # Only include data with length == 400
        if len(data) == 400:
            data_list.append(data)
            # Use the file name (without extension) as a label
            labels.append(file_name.replace(".npy", ""))
        else:
            print(f"Skipping {file_name} as its length is not exactly 400.")
import matplotlib.pyplot as plt
import numpy as np
...
if data_list:
    ...
    plt.savefig("synchronous_server_loss_plot.png", dpi=300, bbox_inches='tight')  # Save with tight bounding box
