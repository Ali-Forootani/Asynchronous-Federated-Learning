...
# -*- coding: utf-8 -*-
"""
Asynchronous Federated Learning with Client Participation Tracking
"""
...
async def federated_learning_async(clients_models, server_model, clients_dataloaders, total_updates=100, local_epochs=1,
    ...
    server_losses = []
    participating_clients_record = []  # Track participating clients in each round
    ...
    while global_updates < total_updates:
        # Select a random subset of clients (once per round)
        selected_clients = random.sample(range(num_clients), num_clients_per_round)
        participating_clients_record.append(selected_clients)  # Log selected clients for this round

        # Create tasks for the selected clients
        tasks = [train_client_convex(clients_models[i], clients_dataloaders[i], device, local_epochs, loss_fn, gamma_0, alpha, delay_t) for i in selected_clients]
        results = await asyncio.gather(*tasks)

        # Update server model after all selected clients have trained
        for i, (state_dict, client_losses) in zip(selected_clients, results):
    ...
    return server_model, server_losses, client_losses_record, participating_clients_record
...
def plot_participating_clients(participating_clients_record, save_path="participating_clients.png"):
    plt.figure(figsize=(10, 6))
    for round_num, clients in enumerate(participating_clients_record):
        plt.scatter([round_num + 1] * len(clients), clients, alpha=0.7)
    
    plt.xlabel("Round Number")
    plt.ylabel("Client ID")
    plt.title("Clients Participating in Each Round")
    plt.grid(True)
    plt.savefig(save_path)
    plt.show()
...
async def main():
    ...
    server_model, server_losses, client_losses_record, participating_clients_record = await federated_learning_async(
    ...
    plot_participating_clients(participating_clients_record)
