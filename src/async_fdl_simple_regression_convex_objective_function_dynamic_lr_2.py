...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 10 09:22:11 2025
...
def simple_linear_data(n_samples=1000, input_dim=10):
    X = np.random.uniform(-5, 5, (n_samples, input_dim))  # Random features in the range [-5, 5]
    true_weights = np.random.uniform(2.0, 4.0, size=input_dim)  # Random true weights
    y = X @ true_weights + 5 + np.random.normal(0, 0.2, size=n_samples)  # Add noise (stddev=0.2)
    return X, y
...
async def train_client_convex(client_model, train_loader, device, local_epochs, loss_fn, gamma_0, alpha, delay_t=0):
    ...
    await asyncio.sleep(delay_simulation)  # Simulate communication delay
...
# Async federated learning function with weighted average aggregation
async def federated_learning_convex(clients_models, server_model, clients_dataloaders, num_rounds=10, local_epochs=1,
    ...
    for round_num in tqdm(range(num_rounds), desc="Federated Rounds"):
        ...
        total_loss = 0
        for data, target in clients_dataloaders[0]:  # Evaluate using the first client’s data for simplicity
